...
// Services
import { adaptFormToRtl } from '../services/FormFieldsSetting';
...
if (I18nManager.isRTL) {
  Form.stylesheet = adaptFormToRtl(Form.stylesheet);
}
...
export class ProfileForm extends Component {
  ...
  confirmDeleteAccount = async (comment) => {
    const { auth, authActions, componentId } = this.props;
  ...
  render() {
    const { forms } = this.state;
    const { isEdit, showTitles, componentId, isLogged, profile } = this.props;
    ...
    return (
      <Fragment>
        <KeyboardAwareScrollView
          ...
          {!isLogged && (
            <SocialLoginLinksBlock componentId={componentId} isRegistration />
          )}
        </KeyboardAwareScrollView>
